const inventors = [
 ...
 let veryOldInventors= inventors.filter((year) => {return year <= 1599})
 ...
 const inventorNames = inventors.map((fullName)=> {return fullName.first, fullName.last})
 ...
 console.log('Exercise 4 Correct Result: ', { first: 'Ada', last: 'Lovelace', year: 1815, passed: 1852 }) //started using mdn and my mind is at ease
 ...
 let firstLast = people.map(full => {
    let [lastName, firstName] = full.split(', ')
    return `${firstName} ${lastName}`}) //asked my dad for help on this one
 ...
 let isAdultPresent = devs.some(({year}) => year < 2004)
 // Enter your solution code here:
 ...
 let isEveryone19OrOlder = devs.every(({year}) => year < 2005 )
 ...
 let idx = comments.findIndex(search => search.id === 123523)
